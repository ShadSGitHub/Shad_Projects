...
def plot(aveScore, aveScore_deque1000, aveScoreTotal):
    ...
    plt.pause(0.001)
    plt.savefig('TR_AAC.png', dpi=400)

## Input Variables
# Agent files must include a class of the same name with a 'get_action' function
# Do not include './' in file path
agent0_file = 'agents/AAC_agent.py'
...
for i_episode in range(1, n_episodes+1):
    ...
    for t in range(max_t):
        actions0, log_prob = players[0].act( observations[0], eps)
        ...
        for index in range(actions0.shape[0]):
            ...
            e = experience(state, top_action, log_prob, reward[0], observations[0], done)
        ...
        if done:
            for ex in memoryOfGame:
                players[0].step(ex.state, ex.action, ex.log_prob, reward[0], ex.next_state, ex.done)
            players[0].learn()
            break
    ...
    torch.save(players[0].qnetwork_local.state_dict(), 'AAC_checkpoint.pth')
