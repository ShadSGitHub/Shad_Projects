...
def plot(aveScore):
    plt.figure(2)
    plt.clf()        
    plt.title('A2C prototype 2')
    plt.xlabel('Episode')
    plt.ylabel('Winning Rate')
    plt.plot(aveScore)    
    plt.pause(0.001)
    plt.savefig('graph_AAC2.png')
    #plt.close()
...
agent0_file = 'agents/AAC2_agent.py'
...
for i_episode in range(1, n_episodes+1):
    ...
    for t in range(max_t):
        ...
        if done:
            for ex in memoryOfGame:
                players[0].step(ex.state, ex.action, reward[0], ex.next_state, ex.done)
    ...
    torch.save(players[0].qnetwork_local.state_dict(), 'AAC2.pth')
